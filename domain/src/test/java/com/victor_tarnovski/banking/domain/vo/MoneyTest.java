...
class MoneyTest {
    ...
    @ParameterizedTest
    @MethodSource("currencyProvider")
    void givenDoubleAndCurrency_whenConstructingMoney_thenAmountAndCurrencyAreSet(Currency currency) {
        // Given
        double value = 12.34;
    ...
    // Then
    assertEquals(4, allocated[0].amount().longValue());
    assertEquals(3, allocated[1].amount().longValue());
    assertEquals(3, allocated[2].amount().longValue());
    }
    ...
    // Then
    assertEquals(2, allocated[0].amount().longValue());
    assertEquals(3, allocated[1].amount().longValue());
    assertEquals(5, allocated[2].amount().longValue());
    }
    ...
    @ParameterizedTest
    @MethodSource("currencyProvider")
    void givenBigDecimal_whenToLong_thenReturnsCorrectLong(Currency currency) {
    // Given
    Money m = new Money(currency);
    BigDecimal value = new BigDecimal("123.45");

    // When
    long result = m.toLong(value);
